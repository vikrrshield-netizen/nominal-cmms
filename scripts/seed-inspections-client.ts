// scripts/seed-inspections-client.ts
// NOMINAL CMMS — Seed kontrolních bodů budovy (klientský Firebase SDK)
// Spusť: npx tsx scripts/seed-inspections-client.ts
...
async function seed() {
  console.log('=== NOMINAL CMMS — Seed kontrolních bodů (klientský SDK) ===\n');
  ...
  for (const t of TEMPLATES) {
    ...
    await setDoc(doc(db, 'inspection_templates', id), {
      ...t,
      isDeleted: false,
      createdAt: now,
      updatedAt: now,
  ...
  const doneIds = [1, 2, 3, 5, 6, 7, 8, 24, 25, 26, 33, 34];
  const defectIds = [3, 8];
  ...
  console.log(`  Závady: ${defectCount}`);
  console.log(`  Čeká: ${pendingCount}`);

  console.log(`\nBudova C: ${TEMPLATES.filter((t) => t.building === 'C').length} místností`);
